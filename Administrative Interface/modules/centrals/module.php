...
/**
 * VoIP Domain centrals module. This module add the feature of key centrals
 * that rings in one or many centrals at same time when called. This is usefull
 ...
 */
framework_add_path ( "/centrals", "centrals_search_page");
framework_add_hook ( "centrals_search_page", "centrals_search_page", IN_HOOK_INSERT_FIRST);

framework_add_path ( "/centrals/add", "centrals_add_page");
framework_add_hook ( "centrals_add_page", "centrals_add_page", IN_HOOK_INSERT_FIRST);

framework_add_path ( "/centrals/:id/view", "centrals_view_page");
framework_add_hook ( "centrals_view_page", "centrals_view_page", IN_HOOK_INSERT_FIRST);

framework_add_path ( "/centrals/:id/edit", "centrals_edit_page");
framework_add_hook ( "centrals_edit_page", "centrals_edit_page", IN_HOOK_INSERT_FIRST);

framework_add_path ( "/centrals/:id/report", "centrals_report_page");
framework_add_hook ( "centrals_report_page", "centrals_report_page", IN_HOOK_INSERT_FIRST);

/**
 * Function to generate the main centrals page.
 *
 ...
 */
function centrals_search_page ( $buffer, $parameters)
{
  global $_in;
   ...
   * Set page title
   */
  sys_set_title ( __ ( "Centrals"));
  sys_set_subtitle ( __ ( "centrals search"));
  sys_set_path ( array (
    1 => array ( "title" => __ ( "Centrals"))
  ));
  ...
  /**
   * Add central remove modal code
   */
  $output .= "<div id=\"central_delete\" class=\"modal fade\" role=\"dialog\" aria-labelledby=\"central_delete\" aria-hidden=\"true\">\n";
  $output .= "  <div class=\"modal-dialog\">\n";
  $output .= "    <div class=\"modal-content\">\n";
  $output .= "      <div class=\"modal-header\">\n";
  $output .= "        <button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">×</button>\n";
  $output .= "        <h3 class=\"modal-title\">" . __ ( "Central remove") . "</h3>\n";
  $output .= "      </div>\n";
  $output .= "      <div class=\"modal-body\"><p>" . sprintf ( __ ( "Are sure you want to remove central %s (%s)?"), "<span id=\"central_delete_name\"></span>", "<span id=\"central_delete_extension\"></span>") . "</p><input type=\"hidden\" id=\"central_delete_id\" value=\"\"></div>\n";
  ...
  sys_addjs ( "$('#datatables').data ( 'dt', $('#datatables').DataTable (\n" .
              "{\n" .
              "  data: VoIP.dataTables ( '/centrals/fetch', 'GET'),\n" .
              "  columnDefs: [\n" .
              "                { orderable: false, targets: [ 0, 3, 4 ]},\n" .
              "                { searchable: false, targets: [ 0, 3, 4 ]},\n" .
              "                { data: 'links', render: function ( data, type, full) { return '<span class=\"btn-group\"><a class=\"btn btn-xs btn-info\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"" . __ ( "View") . "\" role=\"button\" title=\"\" href=\"/centrals/' + full[0] + '/view\"><i class=\"fa fa-search\"></i></a><a class=\"btn btn-xs btn-success\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"" . __ ( "Report") . "\" role=\"button\" title=\"\" href=\"/centrals/' + full[0] + '/report\"><i class=\"fa fa-list\"></i></a><a class=\"btn btn-xs btn-warning\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"" . __ ( "Edit") . "\" role=\"button\" title=\"\" href=\"/centrals/' + full[0] + '/edit\"><i class=\"fa fa-pencil-alt\"></i></a><button class=\"btn btn-xs btn-danger\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"" . __ ( "Remove") . "\" role=\"button\" title=\"\" data-id=\"' + full[0] + '\" data-extension=\"' + full[1] + '\" data-name=\"' + full[2] + '\"><i class=\"fa fa-times\"></i></button></span>'; }, targets: [ 4 ]},\n" .
              "                { visible: false, targets: [ 0 ]}\n" .
              "              ],\n" .
              "  columns: [\n" .
              "             { class: 'never'},\n" .
              "             { width: '10%', class: 'export all'},\n" .
              "             { width: '68%', class: 'export all'},\n" .
              "             { width: '10%', class: 'export min-tablet-l'},\n" .
              "             { width: '12%', class: 'all'}\n" .
              "           ]\n" .
              "}));\n" .
              "$('#fastsearch').val ( '');\n" .
              "$('#addbutton').html ( '<a class=\"btn btn-success\" href=\"/centrals/add\"><i class=\"fa fa-asterisk\"></i> " . __ ( "New") . "</a>').css ( 'margin-right', '5px');\n" .
              "$('#datatables').on ( 'click', 'button.btn-danger', function ( e)\n" .
              "{\n" .
              "  e && e.preventDefault ();\n" .
              "  $('#central_delete_id').data ( 'dtrow', $('#datatables').data ( 'dt').rows ( { filter: 'applied'}).row ( $(this).parents ( 'tr').get ( 0)));\n" .
              "  $('#central_delete button.del').prop ( 'disabled', false);\n" .
              "  $('#central_delete_id').val ( $(this).data ( 'id'));\n" .
              "  $('#central_delete_name').html ( $(this).data ( 'name'));\n" .
              "  $('#central_delete_extension').html ( $(this).data ( 'extension'));\n" .
              "  $('#central_delete').modal ( 'show');\n" .
              "});\n" .
              "$('#central_delete button.del').on ( 'click', function ( event)\n" .
              "{\n" .
              "  var l = Ladda.create ( this);\n" .
              "  l.start ();\n" .
              "  var central = VoIP.rest ( '/centrals/' + $('#central_delete_id').val (), 'DELETE');\n" .
              "  if ( central.API.status == 'ok')\n" .
              "  {\n" .
              "    $('#central_delete').modal ( 'hide');\n" .
              "    new PNotify ( { title: '" . __ ( "Central remove") . "', text: '" . __ ( "Central removed sucessfully!") . "', type: 'success'});\n" .
              "    $('#central_delete_id').data ( 'dtrow').remove ().draw ();\n" .
              "  } else {\n" .
              "    new PNotify ( { title: '" . __ ( "Central remove") . "', text: '" . __ ( "Error removing central!") . "', type: 'error'});\n" .
              "  }\n" .
              "  l.stop ();\n" .
              "});\n");
...
/**
 * Function to generate the central add page code.
 *
 * @param string $buffer Buffer from plugin system if processed by other function
 *                       before
 * @param array $parameters Optional parameters to the function
 * @return string Output of the generated page
 */
function centrals_add_page ( $buffer, $parameters)
{
  /**
   * Set page title
   */
  sys_set_title ( __ ( "Centrals"));
  sys_set_subtitle ( __ ( "centrals addition"));
  sys_set_path ( array (
    1 => array ( "title" => __ ( "Centrals"), "link" => "/centrals"),
   ...
   * Create page code
   */
  $output = "<form class=\"form-horizontal\" id=\"central_add_form\">\n";

  // Add central number field
  $output .= "  <div class=\"form-group\">\n";
  $output .= "    <label for=\"central_add_extension\" class=\"control-label col-xs-2\">" . __ ( "Extension") . "</label>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <input type=\"text\" name=\"extension\" class=\"form-control\" id=\"central_add_extension\" placeholder=\"" . __ ( "Central extension number") . "\" />\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";

  // Add central name field
  $output .= "  <div class=\"form-group\">\n";
  $output .= "    <label for=\"central_add_name\" class=\"control-label col-xs-2\">" . __ ( "Name") . "</label>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <input type=\"text\" name=\"name\" class=\"form-control\" id=\"central_add_name\" placeholder=\"" . __ ( "Central name") . "\" />\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";

  // Add central extensions field
  $output .= "  <div class=\"form-group\">\n";
  $output .= "    <label for=\"central_add_extensions\" class=\"control-label col-xs-2\">" . __ ( "Extensions") . "</label>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <select name=\"extensions\" id=\"central_add_extensions\" class=\"form-control\" data-placeholder=\"" . __ ( "Central extensions") . "\" multiple=\"multiple\"></select>\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";
  ...
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <a class=\"btn btn-default\" href=\"/centrals\" alt=\"\">" . __ ( "Cancel") . "</a>\n";
   ...
   */
  sys_addjs ( "$('#central_add_extensions').select2 (\n" .
              "{\n" .
              "  allowClear: true,\n" .
              "  data: VoIP.select2 ( '/extensions/search', 'GET')\n" .
              "});\n" .
              "$('#central_add_extension').mask ( '0#').focus ();\n" .
              "$('#central_add_form').alerts ( 'form',\n" .
              "{\n" .
              "  form:\n" .
              "  {\n" .
              "    URL: '/centrals',\n" .
              "    method: 'POST',\n" .
              "    button: $('button.add'),\n" .
              "    title: '" . __ ( "Central addition") . "',\n" .
              "    fail: '" . __ ( "Error adding central!") . "',\n" .
              "    success: '" . __ ( "Central added sucessfully!") . "',\n" .
              "    onsuccess: function ()\n" .
              "               {\n" .
              "                 VoIP.path.call ( '/centrals', true);\n" .
              "               }\n" .
              "  }\n" .
              "});");
...
/**
 * Function to generate the central view page code.
 *
 * @global array $_in Framework global configuration variable
 * @param string $buffer Buffer from plugin system if processed by other function
 *                       before
 ...
 * @return string Output of the generated page
 */
function centrals_view_page ( $buffer, $parameters)
{
  global $_in;
   ...
   */
  sys_set_title ( __ ( "Centrals"));
  sys_set_subtitle ( __ ( "centrals visualization"));
  sys_set_path ( array (
    1 => array ( "title" => __ ( "Centrals"), "link" => "/centrals"),
   ...
   * Create page code
   */
  $output = "<form class=\"form-horizontal\" id=\"central_view_form\">\n";

  // Add central number field
  $output .= "  <div class=\"form-group\">\n";
  $output .= "    <label for=\"central_view_extension\" class=\"control-label col-xs-2\">" . __ ( "Extension") . "</label>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <input type=\"text\" name=\"extension\" class=\"form-control\" id=\"central_view_extension\" placeholder=\"" . __ ( "Central extension number") . "\" disabled=\"disabled\" />\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";

  // Add central name field
  $output .= "  <div class=\"form-group\">\n";
  $output .= "    <label for=\"central_view_name\" class=\"control-label col-xs-2\">" . __ ( "Name") . "</label>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <input type=\"text\" name=\"name\" class=\"form-control\" id=\"central_view_name\" placeholder=\"" . __ ( "Central name") . "\" disabled=\"disabled\" />\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";

  // Add central extensions field
  $output .= "  <div class=\"form-group\">\n";
  $output .= "    <label for=\"central_view_extensions\" class=\"control-label col-xs-2\">" . __ ( "Extensions") . "</label>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <select name=\"extensions\" id=\"central_view_extensions\" class=\"form-control\" data-placeholder=\"" . __ ( "Central extensions") . "\" multiple=\"multiple\" disabled=\"disabled\"></select>\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";
  ...
  $output .= "    <div class=\"col-xs-2\"></div>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <a class=\"btn btn-default\" href=\"/centrals\" alt=\"\">" . __ ( "Return") . "</a>\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";
   ...
   */
  sys_addjs ( "$('#central_view_extensions').select2 ();\n" .
              "$('#central_view_form').on ( 'fill', function ( event, data)\n" .
              "{\n" .
              "  $('#central_view_name').val ( data.name);\n" .
              "  $('#central_view_extension').val ( data.extension);\n" .
              "  var ids = [];\n" .
              "  for ( var id in data.extensions)\n" .
              "  {\n" .
              "    if ( ! data.extensions.hasOwnProperty ( id))\n" .
              "    {\n" .
              "      continue;\n" .
              "    }\n" .
              "    ids.push ( id);\n" .
              "    $('#central_view_extensions').append ( $('<option>', { value: id, text: data.extensions[id]}));\n" .
              "  }\n" .
              "  $('#central_view_extensions').val ( ids).trigger ( 'change');\n" .
              "});\n" .
              "setTimeout ( function ()\n" .
              "{\n" .
              "  var central = VoIP.rest ( '/centrals/' + VoIP.parameters.id, 'GET');\n" .
              "  if ( central.API.status === 'ok')\n" .
              "  {\n" .
              "    $('#central_view_form').trigger ( 'fill', central.result);\n" .
              "  } else {\n" .
              "    new PNotify ( { title: '" . __ ( "Central visualization") . "', text: '" . __ ( "Error viewing central!") . "', type: 'error'});\n" .
              "  }\n" .
              "}, 0);\n");
...
/**
 * Function to generate the central edit page code.
 *
 * @global array $_in Framework global configuration variable
 * @param string $buffer Buffer from plugin system if processed by other function
 *                       before
 ...
 * @return string Output of the generated page
 */
function centrals_edit_page ( $buffer, $parameters)
{
  global $_in;
   ...
   */
  sys_set_title ( __ ( "Centrals"));
  sys_set_subtitle ( __ ( "centrals editing"));
  sys_set_path ( array (
    1 => array ( "title" => __ ( "Centrals"), "link" => "/centrals"),
   ...
   * Create page code
   */
  $output = "<form class=\"form-horizontal\" id=\"central_edit_form\">\n";

  // Add central number field
  $output .= "  <div class=\"form-group\">\n";
  $output .= "    <label for=\"central_edit_extension\" class=\"control-label col-xs-2\">" . __ ( "Extension") . "</label>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <input type=\"text\" name=\"extension\" class=\"form-control\" id=\"central_edit_extension\" placeholder=\"" . __ ( "Central extension number") . "\" />\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";

  // Add central name field
  $output .= "  <div class=\"form-group\">\n";
  $output .= "    <label for=\"central_edit_name\" class=\"control-label col-xs-2\">" . __ ( "Name") . "</label>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <input type=\"text\" name=\"name\" class=\"form-control\" id=\"central_edit_name\" placeholder=\"" . __ ( "Central name") . "\" />\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";

  // Add central extensions field
  $output .= "  <div class=\"form-group\">\n";
  $output .= "    <label for=\"central_edit_extensions\" class=\"control-label col-xs-2\">" . __ ( "Extensions") . "</label>\n";
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <select name=\"extensions\" id=\"central_edit_extensions\" class=\"form-control\" data-placeholder=\"" . __ ( "Central extensions") . "\" multiple=\"multiple\"></select>\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";
   ...
   */
  sys_addjs ( "$('#central_edit_extensions').select2 (\n" .
              "{\n" .
              "  allowClear: true,\n" .
              "  data: VoIP.select2 ( '/extensions/search', 'GET')\n" .
              "});\n" .
              "$('#central_edit_extension').mask ( '0#');\n" .
              "$('#central_edit_form').on ( 'fill', function ( event, data)\n" .
              "{\n" .
              "  $('#central_edit_name').val ( data.name);\n" .
              "  $('#central_edit_extension').val ( data.extension);\n" .
              "  var ids = [];\n" .
              "  for ( var id in data.extensions)\n" .
              "  {\n" .
              "    if ( ! data.extensions.hasOwnProperty ( id))\n" .
              ...
              "    ids.push ( id);\n" .
              "  }\n" .
              "  $('#central_edit_extensions').val ( ids).trigger ( 'change');\n" .
              "  $('#central_edit_extension').focus ();\n" .
              "});\n" .
              "setTimeout ( function ()\n" .
              "{\n" .
              "  var central = VoIP.rest ( '/centrals/' + VoIP.parameters.id, 'GET');\n" .
              "  if ( central.API.status === 'ok')\n" .
              "  {\n" .
              "    $('#central_edit_form').trigger ( 'fill', central.result);\n" .
              "  } else {\n" .
              "    new PNotify ( { title: '" . __ ( "Central edition") . "', text: '" . __ ( "Error requesting central data!") . "', type: 'error'});\n" .
              "  }\n" .
              "}, 0);\n" .
              "$('#central_edit_form').alerts ( 'form',\n" .
              "{\n" .
              "  form:\n" .
              "  {\n" .
              "    URL: '/centrals/' + VoIP.parameters.id,\n" .
              "    method: 'PATCH',\n" .
              "    button: $('button.edit'),\n" .
              "    title: '" . __ ( "Central edition") . "',\n" .
              "    fail: '" . __ ( "Error changing central!") . "',\n" .
              "    success: '" . __ ( "Central changed sucessfully!") . "',\n" .
              "    onsuccess: function ()\n" .
              "               {\n" .
              "                 VoIP.path.call ( '/centrals', true);\n" .
              "               }\n" .
...
/**
 * Function to generate the central report page code.
 *
 ...
 * @return string Output of the generated page
 */
function centrals_report_page ( $buffer, $parameters)
{
  /**
   * Set page title
   */
  sys_set_title ( __ ( "Centrals"));
  sys_set_subtitle ( __ ( "centrals report"));
  sys_set_path ( array (
    1 => array ( "title" => __ ( "Centrals"), "link" => "/centrals"),
  ...
  sys_addjs ( "$('#call_view_server').select2 ();\n" .
              ...
              "  $('#report').trigger ( 'update', VoIP.rest ( '/centrals/' + VoIP.parameters.id + '/report', 'GET', { start: $('#start').val (), end: $('#end').val ()}));\n" .
              "});\n" .
