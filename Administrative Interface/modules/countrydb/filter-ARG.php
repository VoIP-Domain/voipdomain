...
function e164_identify_country_ARG ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $data)
  {
    if ( (int) substr ( $parameters["Number"], 3, strlen ( $prefix)) == $prefix)
    {
      ...
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "54", "NDC" => (string) $prefix, "Country" => "Argentina", "Area" => $data["area"], "City" => $data["city"], "Operator" => "", "SN" => substr ( $parameters["Number"], 3 + strlen ( $prefix)), "Type" => VD_CALLENDPOINT_MOBILE | VD_CALLENDPOINT_LANDLINE, "CallFormats" => $callformats));
    }
  }
