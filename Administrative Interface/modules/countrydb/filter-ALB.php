...
function e164_identify_country_ALB ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    if ( (int) substr ( $parameters["Number"], 4, strlen ( $prefix)) == $prefix)
    {
      ...
      {
        return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "355", "NDC" => substr ( $parameters["Number"], 4, 2), "Country" => "Albania", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 6), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => "0" . substr ( $parameters["Number"], 4, 2) . " " . substr ( $parameters["Number"], 6, 3) . "-" . substr ( $parameters["Number"], 9), "International" => "+355 " . substr ( $parameters["Number"], 4, 2) . " " . substr ( $parameters["Number"], 6, 3) . " " . substr ( $parameters["Number"], 9))));
      } else {
        return ( is_array ( $buffer) ? $buffer : false);
      }
    }
  ...
  foreach ( $prefixes as $prefix => $area)
  {
    if ( (int) substr ( $parameters["Number"], 4, strlen ( $prefix)) == $prefix)
    {
      if ( strlen ( $parameters["Number"]) == 12 && (int) substr ( $parameters["Number"], 4 + strlen ( $prefix), 1) >= 2 && (int) substr ( $parameters["Number"], 4 + strlen ( $prefix), 1) <= 9)
      {
        ...
        return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "355", "NDC" => $prefix, "Country" => "Albania", "Area" => $area, "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], 4 + strlen ( $prefix)), "Type" => VD_CALLENDPOINT_LANDLINE, "CallFormats" => $callformats));
      }
    }
  }
