...
function e164_identify_country_NG_VOX ( $buffer, $parameters)
{
  ...
  {
    return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "8835100", "NDC" => "", "Country" => "Voxbone SA", "Area" => "", "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], 8), "Type" => VD_CALLENDPOINT_SATELLITE, "CallFormats" => array ( "Local" => "0" . substr ( $parameters["Number"], 8, 2) . " " . substr ( $parameters["Number"], 10), "International" => "+883 510 0" . substr ( $parameters["Number"], 8, 2) . " " . substr ( $parameters["Number"], 10))));
  } else {
    return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "8835100", "NDC" => "", "Country" => "Voxbone SA", "Area" => "", "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], 8), "Type" => VD_CALLENDPOINT_SATELLITE, "CallFormats" => array ( "Local" => "0" . substr ( $parameters["Number"], 8, 2) . " " . substr ( $parameters["Number"], 10, 3) . " " . substr ( $parameters["Number"], 13), "International" => "+883 510 0" . substr ( $parameters["Number"], 8, 2) . " " . substr ( $parameters["Number"], 10, 3) . " " . substr ( $parameters["Number"], 13))));
  }
}
?>
