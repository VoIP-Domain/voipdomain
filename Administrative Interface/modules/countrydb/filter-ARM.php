...
function e164_identify_country_ARM ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $data)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "374", "NDC" => substr ( $parameters["Number"], 4, ( strlen ( $prefix) == 2 ? 2 : 3)), "Country" => "Armenia", "Area" => $data["area"], "City" => $data["city"], "Operator" => $data["operator"], "SN" => substr ( $parameters["Number"], ( strlen ( $prefix) == 2 ? 6 : 7)), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => "(" . substr ( $parameters["Number"], 4, ( strlen ( $prefix) == 2 ? 2 : 3)) . ") " . substr ( $parameters["Number"], 4 + ( strlen ( $prefix) == 2 ? 2 : 3)), "International" => "+374 " . substr ( $parameters["Number"], 4, ( strlen ( $prefix) == 2 ? 2 : 3)) . " " . substr ( $parameters["Number"], 4 + ( strlen ( $prefix) == 2 ? 2 : 3)))));
    }
  }
