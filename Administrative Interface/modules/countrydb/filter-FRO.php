...
function e164_identify_country_FRO ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "298", "NDC" => substr ( $parameters["Number"], 4, 2), "Country" => "Faroe Islands", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 6), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 4), "International" => "+298 " . substr ( $parameters["Number"], 4))));
    }
  }
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    if ( (int) substr ( $parameters["Number"], 4, strlen ( $prefix)) == $prefix)
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "298", "NDC" => substr ( $parameters["Number"], 4, 2), "Country" => "Faroe Islands", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 6), "Type" => VD_CALLENDPOINT_VOIP, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 4), "International" => "+298 " . substr ( $parameters["Number"], 4))));
    }
  }
