...
function e164_identify_country_ECU ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "593", "NDC" => substr ( $parameters["Number"], 4, 2), "Country" => "Ecuador", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 6), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 4, 1) . " " . substr ( $parameters["Number"], 5, 1) . " " . substr ( $parameters["Number"], 6), "International" => "+593 " . substr ( $parameters["Number"], 4, 1) . " " . substr ( $parameters["Number"], 5, 1) . " " . substr ( $parameters["Number"], 6))));
    }
  }
