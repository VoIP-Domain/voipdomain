...
function e164_identify_country_BEL ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "32", "NDC" => substr ( $parameters["Number"], 3, 3), "Country" => "Belgian", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 6), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => "0" . substr ( $parameters["Number"], 3, 3) . "-" . substr ( $parameters["Number"], 6, 2) . "-" . substr ( $parameters["Number"], 8, 2) . "-" . substr ( $parameters["Number"], 10), "International" => "+32 " . substr ( $parameters["Number"], 3, 3) . " " . substr ( $parameters["Number"], 6, 2) . " " . substr ( $parameters["Number"], 8, 2) . " " . substr ( $parameters["Number"], 10))));
    }
  }
  ...
  foreach ( $prefixes as $prefix => $area)
  {
    if ( (int) substr ( $parameters["Number"], 3, strlen ( $prefix)) == $prefix && strlen ( $parameters["Number"]) == 11)
    {
      ...
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "32", "NDC" => (string) $prefix, "Country" => "Belgian", "Area" => $area, "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], ( strlen ( $prefix) == 1 ? 4 : 5)), "Type" => VD_CALLENDPOINT_LANDLINE, "CallFormats" => $callformats));
    }
  }
  ...
  {
    return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "32", "NDC" => "70", "Country" => "Belgian", "Area" => "", "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], 5), "Type" => VD_CALLENDPOINT_SPECIAL, "CallFormats" => array ( "Local" => "0" . substr ( $parameters["Number"], 3, 2) . "-" . substr ( $parameters["Number"], 5, 3) . "-" . substr ( $parameters["Number"], 8), "International" => "+32 " . substr ( $parameters["Number"], 3, 2) . " " . substr ( $parameters["Number"], 5, 3) . " " . substr ( $parameters["Number"], 8))));
  }
