...
function e164_identify_country_ASM ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $data)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "1684", "NDC" => substr ( $parameters["Number"], 5, 3), "Country" => "American Samoa", "Area" => $data["area"], "City" => "", "Operator" => $data["operator"], "SN" => substr ( $parameters["Number"], 8), "Type" => ( substr ( $parameters["Number"], 5, 1) == "2" || substr ( $parameters["Number"], 5, 1) == "7" ? VD_CALLENDPOINT_MOBILE : VD_CALLENDPOINT_LANDLINE), "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 5, 3) . " " . substr ( $parameters["Number"], 8), "International" => "+1 684 " . substr ( $parameters["Number"], 5, 3) . " " . substr ( $parameters["Number"], 8))));
    }
  }
