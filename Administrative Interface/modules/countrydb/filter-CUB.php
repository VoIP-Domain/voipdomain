...
function e164_identify_country_CUB ( $buffer, $parameters)
{
  ...
  {
    return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "53", "NDC" => substr ( $parameters["Number"], 3, 1), "Country" => "Cuba", "Area" => "", "City" => "", "Operator" => "Cubacel", "SN" => substr ( $parameters["Number"], 4), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 3, 4) . " " . substr ( $parameters["Number"], 7), "International" => "+53 " . substr ( $parameters["Number"], 3, 4) . " " . substr ( $parameters["Number"], 7))));
  }
  ...
  foreach ( $prefixes as $prefix => $data)
  {
    if ( (int) substr ( $parameters["Number"], 3, strlen ( $prefix)) == $prefix && strlen ( $parameters["Number"]) >= $data["minimum"] + 3 && strlen ( $parameters["Number"]) <= $data["maximum"] + 3)
    {
      ...
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "53", "NDC" => (string) $prefix, "Country" => "Cuba", "Area" => $data["area"], "City" => $data["city"], "Operator" => "Empresa de Telecomunicaciones de Cuba S.A.", "SN" => substr ( $parameters["Number"], 3 + strlen ( $prefix)), "Type" => VD_CALLENDPOINT_LANDLINE, "CallFormats" => $callformats));
    }
  }
