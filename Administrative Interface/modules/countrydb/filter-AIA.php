...
function e164_identify_country_AIA ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "1264", "NDC" => substr ( $parameters["Number"], 5, 3), "Country" => "Anguilla", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 8), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 5, 3) . " " . substr ( $parameters["Number"], 8), "International" => "+1 264 " . substr ( $parameters["Number"], 5, 3) . " " . substr ( $parameters["Number"], 8))));
    }
  }
