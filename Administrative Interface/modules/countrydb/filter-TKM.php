...
function e164_identify_country_TKM ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "993", "NDC" => $prefix, "Country" => "Turkmenistan", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 4 + strlen ( $prefix)), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 4, 1) . " " . substr ( $parameters["Number"], 5), "International" => "+993 " . substr ( $parameters["Number"], 4, 1) . " " . substr ( $parameters["Number"], 5))));
    }
  }
