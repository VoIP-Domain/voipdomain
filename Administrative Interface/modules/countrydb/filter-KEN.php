...
function e164_identify_country_KEN ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "254", "NDC" => substr ( $parameters["Number"], 4, 3), "Country" => "Kenya", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 7), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 4, 3) . " " . substr ( $parameters["Number"], 7), "International" => "+254 " . substr ( $parameters["Number"], 4, 3) . " " . substr ( $parameters["Number"], 7))));
    }
  }
