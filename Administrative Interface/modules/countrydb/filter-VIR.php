...
function e164_identify_country_VIR ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $data)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "1340", "NDC" => substr ( $parameters["Number"], 5, 3), "Country" => "Virgin Islands (U.S.)", "Area" => $data["Area"], "City" => $data["City"], "Operator" => "", "SN" => substr ( $parameters["Number"], 8), "Type" => VD_CALLENDPOINT_MOBILE + VD_CALLENDPOINT_LANDLINE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 5, 3) . " " . substr ( $parameters["Number"], 8), "International" => "+1 340 " . substr ( $parameters["Number"], 5, 3) . " " . substr ( $parameters["Number"], 8))));
    }
  }
