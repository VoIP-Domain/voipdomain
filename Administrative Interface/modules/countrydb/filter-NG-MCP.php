...
function e164_identify_country_NG_MCP ( $buffer, $parameters)
{
   ...
   */
  return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "88232", "NDC" => "", "Country" => "Maritime Communications Partner (MCP)", "Area" => "", "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], 6), "Type" => VD_CALLENDPOINT_SATELLITE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 6, 4) . " " . substr ( $parameters["Number"], 10), "International" => "+882 32 " . substr ( $parameters["Number"], 6, 4) . " " . substr ( $parameters["Number"], 10))));
}
?>
