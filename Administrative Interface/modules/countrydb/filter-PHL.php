...
function e164_identify_country_PHL ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "63", "NDC" => substr ( $parameters["Number"], 3, 3), "Country" => "Philippines", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 6), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => "0 " . substr ( $parameters["Number"], 3, 3) . " " . substr ( $parameters["Number"], 6, 3) . "-" . substr ( $parameters["Number"], 9), "International" => "+63 " . substr ( $parameters["Number"], 3, 3) . " " . substr ( $parameters["Number"], 6, 3) . " " . substr ( $parameters["Number"], 9))));
    }
  }
  ...
  foreach ( $prefixes as $prefix => $data)
  {
    if ( ( strlen ( $parameters["Number"]) == 11 || strlen ( $parameters["Number"]) == 12) && (int) substr ( $parameters["Number"], 3, strlen ( $prefix)) == $prefix)
    {
      ...
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "63", "NDC" => $ndc, "Country" => "Philippines", "Area" => $data["Area"], "City" => $data["City"], "Operator" => $operator, "SN" => $sn, "Type" => VD_CALLENDPOINT_LANDLINE, "CallFormats" => array ( "Local" => $local, "International" => $international)));
    }
  }
