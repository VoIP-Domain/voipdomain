...
function e164_identify_country_BGR ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $digits)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "359", "NDC" => substr ( $parameters["Number"], 4, 2), "Country" => "Bulgaria", "Area" => "", "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], 6), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => "(0" . substr ( $parameters["Number"], 4, 2) . ") " . substr ( $parameters["Number"], 6, 4) . " " . substr ( $parameters["Number"], 10), "International" => "+359 " . substr ( $parameters["Number"], 4, 2) . " " . substr ( $parameters["Number"], 6, 4) . " " . substr ( $parameters["Number"], 10))));
    }
  }
  ...
  foreach ( $prefixes as $prefix => $area)
  {
    if ( (int) substr ( $parameters["Number"], 4, strlen ( $prefix)) == $prefix && strlen ( $parameters["Number"]) == 12)
    {
      ...
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "359", "NDC" => substr ( $parameters["Number"], 4, ( $prefix == "2" ? 1 : 2)), "Country" => "Bulgaria", "Area" => $area, "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], ( $prefix == "2" ? 5 : 6)), "Type" => VD_CALLENDPOINT_LANDLINE, "CallFormats" => $callformats));
    }
  }
