...
function e164_identify_country_AGO ( $buffer, $parameters)
{
  ...
  if ( substr ( $parameters["Number"], 4, 1) == "9" && strlen ( $parameters["Number"]) == 13)
  {
    ...
    return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "244", "NDC" => substr ( $parameters["Number"], 4, 3), "Country" => "Angola", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 7), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 4, 3) . " " . substr ( $parameters["Number"], 7, 3) . " " . substr ( $parameters["Number"], 10, 3), "International" => "+244 " . substr ( $parameters["Number"], 4, 3) . " " . substr ( $parameters["Number"], 7, 3) . substr ( $parameters["Number"], 10, 3))));
  }
