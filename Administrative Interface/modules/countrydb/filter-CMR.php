...
function e164_identify_country_CMR ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $operator)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "237", "NDC" => substr ( $parameters["Number"], 4, 2), "Country" => "Cameroon", "Area" => "", "City" => "", "Operator" => $operator, "SN" => substr ( $parameters["Number"], 6), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 4, 2) . " " . substr ( $parameters["Number"], 6, 3) . " " . substr ( $parameters["Number"], 9), "International" => "+237 " . substr ( $parameters["Number"], 4, 2) . " " . substr ( $parameters["Number"], 6, 3) . " " . substr ( $parameters["Number"], 9))));
    }
  }
  ...
  foreach ( $prefixes as $prefix => $data)
  {
    if ( (int) substr ( $parameters["Number"], 4, strlen ( $prefix)) == $prefix)
    {
      switch ( substr ( $parameters["Number"], 4, 3))
      {
        case "222":
        case "233":
          $type = VD_CALLENDPOINT_LANDLINE;
          break;
        case "242":
        case "243":
          $type = VD_CALLENDPOINT_FMC;
          break;
        default:
          $type = VD_CALLENDPOINT_LANDLINE;
          break;
