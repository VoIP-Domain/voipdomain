...
function e164_identify_country_SLV ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "503", "NDC" => substr ( $parameters["Number"], 4, 1), "Country" => "El Salvador", "Area" => "", "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], 5), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 4, 4) . " " . substr ( $parameters["Number"], 8), "International" => "+503 " . substr ( $parameters["Number"], 4, 4) . " " . substr ( $parameters["Number"], 8))));
    }
  }
