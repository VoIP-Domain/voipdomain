...
function e164_identify_country_NIU ( $buffer, $parameters)
{
  ...
  foreach ( $prefixes as $prefix => $length)
  {
    if ( strlen ( $parameters["Number"]) == $length + 4 && (int) substr ( $parameters["Number"], 4, strlen ( $prefix)) == $prefix)
    {
      ...
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "683", "NDC" => "", "Country" => "Niue", "Area" => "", "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], 4), "Type" => VD_CALLENDPOINT_MOBILE, "CallFormats" => array ( "Local" => $local, "International" => "+683 " . $local)));
    }
  }
  ...
  foreach ( $prefixes as $prefix)
  {
    ...
    {
      return array_merge_recursive ( ( is_array ( $buffer) ? $buffer : array ()), array ( "CC" => "683", "NDC" => "", "Country" => "Niue", "Area" => "", "City" => "", "Operator" => "", "SN" => substr ( $parameters["Number"], 4), "Type" => VD_CALLENDPOINT_LANDLINE, "CallFormats" => array ( "Local" => substr ( $parameters["Number"], 4), "International" => "+683 " . substr ( $parameters["Number"], 4))));
    }
  }
