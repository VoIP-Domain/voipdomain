...
function notifications_events_search ( $buffer, $parameters)
{
  ...
  $parameters["function"] = array (
    "DefaultFields" => "ID,Event,Description",
    "PermittedFields" => "ID,Event,EventEN,Description,DescriptionEN"
  );
...
function notifications_events_view ( $buffer, $parameters)
{
  ...
  foreach ( $_in["events"][$parameters["ID"]]["Fields"] as $field)
  {
    if ( $field["Type"] == "enum")
    {
      ...
      $data["Fields"][] = array ( "Variable" => $field["Variable"], "Name" => __ ( $field["Name"]), "NameEN" => $field["Name"], "Type" => $field["Type"], "APIPath" => array_key_exists ( "APIPath", $field) ? $field["APIPath"] : "", "TypeEnum" => $enum);
    } else {
      $data["Fields"][] = array ( "Variable" => $field["Variable"], "Name" => __ ( $field["Name"]), "NameEN" => $field["Name"], "Type" => $field["Type"], "APIPath" => array_key_exists ( "APIPath", $field) ? $field["APIPath"] : "");
    }
  }
...
function notifications_search ( $buffer, $parameters)
{
   ...
   */
  $parameters["function"] = array (
    "DefaultFields" => "ID,Description,Event,Expire,ExpireTimestamp",
    "PermittedFields" => "ID,Description,Event,EventEN,Expire,ExpireTimestamp"
  );
...
framework_add_hook (
  ...
  array (
    ...
    "response" => array (
      201 => array (
        "description" => __ ( "New system notification added successfully.")
      ),
      422 => array (
        "description" => __ ( "An error occurred while processing the request. An object with field name and a text error message will be returned to all inconsistency found."),
        "schema" => array (
...
function notifications_add ( $buffer, $parameters)
{
   ...
   * Sanitize parameters
   */
  $parameters["Validity"] = array_key_exists ( "Validity", $parameters) && ! empty ( $parameters["Validity"]) ? format_form_date ( $parameters["Validity"]) . " 00:00:00" : "0000-00-00 00:00:00";
  ...
  $gm->addServer ( array_key_exists ( "hostname", $_in["gearman"]) ? $_in["gearman"]["hostname"] : "127.0.0.1", array_key_exists ( "port", $_in["gearman"]) ? $_in["gearman"]["port"] : 4730);
  $gm->doBackground ( "notification_create", json_encode ( array ( "ID" => $parameters["ID"], "Description" => $parameters["Description"], "Event" => $parameters["Event"], "Filters" => $parameters["Filters"], "URL" => $parameters["URL"], "Method" => $parameters["Method"], "Type" => $parameters["Type"], "Variables" => $parameters["Variables"], "Headers" => $parameters["Headers"], "RelaxSSL" => $parameters["SSL"], "Expire" => $parameters["Validity"])));
...
framework_add_hook (
  ...
  array (
    ...
    "response" => array (
      200 => array (
        "description" => __ ( "The system notification was successfully updated.")
      ),
      422 => array (
        "description" => __ ( "An error occurred while processing the request. An object with field name and a text error message will be returned to all inconsistency found."),
        "schema" => array (
...
function notifications_edit ( $buffer, $parameters)
{
  ...
  $gm = new GearmanClient ();
  $gm->addServer ( array_key_exists ( "hostname", $_in["gearman"]) ? $_in["gearman"]["hostname"] : "127.0.0.1", array_key_exists ( "port", $_in["gearman"]) ? $_in["gearman"]["port"] : 4730);
  $gm->doBackground ( "notification_change", json_encode ( array ( "ID" => $parameters["ID"], "Description" => $parameters["Description"], "Event" => $parameters["Event"], "Filters" => $parameters["Filters"], "URL" => $parameters["URL"], "Method" => $parameters["Method"], "Type" => $parameters["Type"], "Variables" => $parameters["Variables"], "Headers" => $parameters["Headers"], "RelaxSSL" => array_key_exists ( "SSL", $parameters) ? $parameters["SSL"] : "", "Expire" => $parameters["Validity"])));
...
function notifications_remove ( $buffer, $parameters)
{
  ...
  $gm = new GearmanClient ();
  $gm->addServer ( array_key_exists ( "hostname", $_in["gearman"]) ? $_in["gearman"]["hostname"] : "127.0.0.1", array_key_exists ( "port", $_in["gearman"]) ? $_in["gearman"]["port"] : 4730);
  $gm->doBackground ( "notification_destroy", json_encode ( $parameters["ORIGINAL"]));
