...
/**
 * VoIP Domain exceptions module WebUI. This module add the feature to bypass
 * password requirement for a list of numbers. Usefull to add company tech mobile
 * numbers, and others.
 *
...
function exceptions_search_page ( $buffer, $parameters)
{
  ...
  $output .= "      </div>\n";
  $output .= "      <div class=\"modal-body\"><p>" . sprintf ( __ ( "Are you sure you want to remove the exception %s (%s)?"), "<span id=\"exception_delete_description\"></span>", "<span id=\"exception_delete_number\"></span>") . "</p><input type=\"hidden\" id=\"exception_delete_id\" value=\"\"></div>\n";
  ...
  sys_addjs ( "$('#datatables').data ( 'dt', $('#datatables').DataTable (\n" .
              ...
              "             { data: 'Number', title: '" . __ ( "Number") . "', width: '20%', class: 'export all'},\n" .
              "             { data: 'NULL', title: '" . __ ( "Actions") . "', width: '10%', class: 'all', render: function ( data, type, row, meta) { return '<span class=\"btn-group\"><a class=\"btn btn-xs btn-info\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"" . __ ( "View") . "\" role=\"button\" title=\"\" href=\"/exceptions/' + row.ID + '/view\"><i class=\"fa fa-search\"></i></a><a class=\"btn btn-xs btn-warning\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"" . __ ( "Edit") . "\" role=\"button\" title=\"\" href=\"/exceptions/' + row.ID + '/edit\"><i class=\"fa fa-pencil-alt\"></i></a><a class=\"btn btn-xs btn-primary\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"" . __ ( "Clone") . "\" role=\"button\" title=\"\" href=\"/exceptions/' + row.ID + '/clone\" data-nohistory=\"nohistory\"><i class=\"fa fa-clone\"></i></a><button class=\"btn btn-xs btn-danger\" data-toggle=\"tooltip\" data-placement=\"top\" data-original-title=\"" . __ ( "Remove") . "\" role=\"button\" title=\"\" data-id=\"' + row.ID + '\" data-description=\"' + row.Description + '\" data-number=\"' + row.Number + '\"><i class=\"fa fa-times\"></i></button></span>'; }, orderable: false, searchable: false}\n" .
              "           ]\n" .
              "}));\n" .
              "VoIP.dataTablesUpdate ( { path: '/exceptions', fields: 'ID,Description,Number,NULL'}, $('#datatables').data ( 'dt'));\n" .
              ...
              "  l.start ();\n" .
              "  VoIP.rest ( '/exceptions/' + encodeURIComponent ( $('#exception_delete_id').val ()), 'DELETE').done ( function ( data, textStatus, jqXHR)\n" .
              "  {\n" .
...
function exceptions_add_page ( $buffer, $parameters)
{
  ...
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <input type=\"tel\" name=\"Number\" class=\"form-control\" id=\"exception_add_number\" placeholder=\"" . __ ( "Exception number (E.164 standard)") . "\" maxlength=\"16\" />\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";
  ...
  sys_addjs ( "$('#exception_add_number').mask ( '+0#');\n" .
              ...
              "  form:\n" .
              "  {\n" .
              "    focus: $('#exception_add_description'),\n" .
              "    URL: '/exceptions',\n" .
              "    method: 'POST',\n" .
              "    button: $('button.add'),\n" .
              "    title: '" . __ ( "Exception addition") . "',\n" .
              "    fail: '" . __ ( "Error adding exception!") . "',\n" .
              "    success: '" . __ ( "Exception added successfully!") . "',\n" .
              "    onsuccess: function ()\n" .
              "               {\n" .
...
/**
 * Function to generate the exception clone function code.
 *
 ...
 * @return string Output of the generated page
 */
function exceptions_clone_function ( $buffer, $parameters)
{
  global $_in;
   ...
   */
  sys_addjs ( "VoIP.rest ( '/exceptions/' + encodeURIComponent ( VoIP.parameters.id), 'GET').done ( function ( data, textStatus, jqXHR)\n" .
              "{\n" .
              "  VoIP.path.call ( '/exceptions/add', true, function ()\n" .
              "  {\n" .
              "    $('#exception_add_description').val ( data.Description);\n" .
              "    $('#exception_add_number').val ( data.Number);\n" .
              "  });\n" .
              "}).fail ( function ( jqXHR, textStatus, errorThrown)\n" .
              "{\n" .
...
function exceptions_view_page ( $buffer, $parameters)
{
  ...
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <input type=\"tel\" name=\"Number\" class=\"form-control\" id=\"exception_view_number\" placeholder=\"" . __ ( "Exception number (E.164 standard)") . "\" disabled=\"disabled\" />\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";
  ...
  sys_addjs ( "$('#exception_view_form').on ( 'fill', function ( event, data)\n" .
              ...
              "});\n" .
              "VoIP.rest ( '/exceptions/' + encodeURIComponent ( VoIP.parameters.id), 'GET').done ( function ( data, textStatus, jqXHR)\n" .
              "{\n" .
              "  $('#exception_view_form').trigger ( 'fill', data);\n" .
              "}).fail ( function ( jqXHR, textStatus, errorThrown)\n" .
              "{\n" .
...
function exceptions_edit_page ( $buffer, $parameters)
{
  ...
  $output .= "    <div class=\"col-xs-10\">\n";
  $output .= "      <input type=\"tel\" name=\"Number\" class=\"form-control\" id=\"exception_edit_number\" placeholder=\"" . __ ( "Exception number (E.164 standard)") . "\" maxlength=\"16\" />\n";
  $output .= "    </div>\n";
  $output .= "  </div>\n";
  ...
  sys_addjs ( "$('#exception_edit_number').mask ( '+0#');\n" .
              ...
              "    success: '" . __ ( "Exception successfully changed!") . "',\n" .
              "    onsuccess: function ()\n" .
              "               {\n" .
              "                 VoIP.path.call ( '/exceptions', true);\n" .
              "               }\n" .
