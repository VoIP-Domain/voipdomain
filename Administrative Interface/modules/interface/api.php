...
/**
 * Add framework session control login endpoint
 */
framework_add_hook (
  "user_login",
  "user_login",
  ...
  array (
    ...
    "response" => array (
      201 => array (
        "description" => __ ( "New system user session created successfully."),
        "headers" => array (
          "Set-Cookie" => array (
            "schema" => array (
              "type" => "string"
...
/**
 * Add framework session control logout endpoint
 */
...
function dashboard_information ( $page, $parameters)
{
   ...
   * Sanityze parameters
   */
  $parameters["Start"] = strtotime ( array_key_exists ( "Start", $parameters) ? format_form_date ( $parameters["Start"]) : "29 days ago");
  $parameters["End"] = array_key_exists ( "End", $parameters) ? strtotime ( format_form_date ( $parameters["End"])) : mktime ();
   ...
   * Return structured data
   */
  return array_merge_recursive ( isset ( $buffer) && is_array ( $buffer) ? $buffer : array (), $data);
}
...
function fastsearch ( $buffer, $parameters)
{
  ...
  $parameters["function"] = array (
    "DefaultFields" => "ID,Number,Type,Description",
    "PermittedFields" => "ID,Number,Type,Description"
  );
