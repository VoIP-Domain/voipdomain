...
function debug_api_export ( $buffer, $parameters)
{
  ...
  foreach ( $_api as $path => $data)
  {
    ...
    {
      if ( $methoddata["documented"] === false)
      {
        continue;
      }
      ...
      foreach ( $methoddata["permissions"] as $permission)
      {
        switch ( $permission)
        {
          ...
          case "Internal":
            break;
          case "AuthToken":
            if ( ! array_key_exists ( "UserAuthToken", $permissions))
            {
              $permissions["UserAuthToken"] = array ();
            }
            $permissions["UserAuthToken"][] = $permission;
            break;
          default:
            if ( framework_has_hook ( "debug_api_export_permission_" . $permission))
            {
              $permissions = framework_call ( "debug_api_export_permission_" . $permission, $parameters);
            } else {
      ...
      if ( $method == "post")
      {
        $paths[$newpath][$method]["responses"]["201"] = array ( "description" => __ ( "Object successfully created."), "headers" => array ( "Location" => array ( "schema" => array ( "type" => "string"), "description" => __ ( "The URL location of the created object."))));
      }
      if ( $method == "delete")
      {
        $paths[$newpath][$method]["responses"]["204"] = array ( "description" => __ ( "Object successfully removed."));
      }
      ...
      foreach ( $_plugins["hooks"][$methoddata["hook"]] as $function)
      {
        foreach ( $_plugins["functions"][$function]["response"] as $code => $response)
        {
          ...
          {
            $paths[$newpath][$method]["responses"][$code]["headers"] = array_merge_recursive ( $paths[$newpath][$method]["responses"][$code]["headers"], $response["headers"]);
          }
  ...
  if ( ( is_array ( $buffer) && sizeof ( $buffer) == 0) || empty ( $buffer))
  {
    ...
    $data["components"]["securitySchemes"]["ApiKeyAuth"] = array ( "type" => "apiKey", "name" => "X-" . strtoupper ( $_in["general"]["cookie"]) . "-Token", "in" => "header");
