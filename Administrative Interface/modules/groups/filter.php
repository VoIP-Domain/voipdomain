...
{
  return array_merge ( (array) $buffer, array ( array ( "type" => "entry", "icon" => "users", "href" => "/groups", "text" => __ ( "Groups"), "permissions" => array ( "Administrator"))));
}
...
function get_groups ( $buffer, $parameters)
{
   ...
   */
  $data = array ();
  if ( $result = @$_in["mysql"]["id"]->query ( "SELECT * FROM `Groups` WHERE `Tenant` = " . (int) get_tenant () . $where))
  {
...
function groups_profile ( $buffer, $parameters)
{
   ...
   */
  if ( ! $result = @$_in["mysql"]["id"]->query ( "SELECT `ID` FROM `Groups` WHERE `Tenant` = " . (int) get_tenant () . " AND `Profile` = " . (int) $parameters["ID"]))
  {
