...
while ( true)
{
  ...
  while ( substr ( $body, 0, "19") == "a:1:{s:5:\"event\";s:")
  {
    ...
    {
      if ( ! array_key_exists ( "event", $event) || ! array_key_exists ( "id", $event) || ! array_key_exists ( "data", $event))
      {
        writeLog ( "Received event without event name and/or ID and/or data. Ignoring.", VoIP_LOG_WARNING);
      } else {
        if ( array_key_exists ( $event["id"], $processed))
        {
          writeLog ( $event["event"] . "(" . $event["id"] . ") - Ignoring already processed event.", VoIP_LOG_WARNING);
        } else {
          $result = framework_call ( $event["event"], $event["data"]);
