...
function queues_join ( $buffer, $parameters)
{
  ...
  {
    return array ( "code" => 403, "message" => "Invalid parameters.");
  }

  // Verify if queue exist
  if ( ! file_exists ( $_in["general"]["confdir"] . "/queue-" . (int) $parameters["Queue"] . ".conf"))
  {
  $parameters["Agent"] &&   return array ( "code" => 400, "message" => "Queue doesn't exist.");
  }
  ...
  {
    return array ( "code" => 401, "message" => "Extension doesn't exist.");
  }

  // If agent is provided, check if it exist
  if ( $parameters["Agent"] && ! file_exists ( $_in["general"]["confdir"] . "/agent-" . (int) $parameters["Agent"] . ".conf"))
  {
    return array ( "code" => 402, "message" => "Agent doesn't exist.");
  }

  // If agent is provided, login agent to extension, otherwise, add member extensions to queue
  if ( $parameters["Agent"])
  {
    // Login agent to the extension
    asterisk_exec ( "channel originate local/" . (int) $parameters["Extension"] . "@VoIPDomain-extensions application AgentLogin " . (int) $parameters["Agent"]);

    // Add agent to the queue
    asterisk_exec ( "queue add member Agent/" . (int) $parameters["Agent"] . " to queue_" . (int) $parameters["Queue"]);
  } else {
