...
/**
 * If requesting user authentication page, provide it
 */
if ( $_SERVER["REQUEST_URI"] == "/auth" || substr ( $_SERVER["REQUEST_URI"], 0, 6) == "/auth?")
{
  echo framework_call ( "authentication_page_generate");
  exit ();
}
...
foreach ( $_paths as $entrypath => $entry)
{
  ...
  $return["hook"] = $hook;
  $return["type"] = array_key_exists ( "type", $_in["page"]) ? $_in["page"]["type"] : "page";
