...
/**
 * Initialize reserved permissions variables
 */
$_in["reservedpermissions"] = array ();
$_in["reservedpermissions"][] = "Internal";
$_in["reservedpermissions"][] = "Install";
$_in["reservedpermissions"][] = "Token";
$_in["reservedpermissions"][] = "AuthToken";
$_in["reservedpermissions"][] = "User";
$_in["reservedpermissions"][] = "Administrator";
...
define ( "VD_CALLENDPOINT_VSAT", 8192);
define ( "VD_CALLENDPOINT_VOIP", 16384);
define ( "VD_CALLENDPOINT_PAGING", 32768);
define ( "VD_CALLENDPOINT_FMC", 65536);
define ( "VD_CALLENDPOINT_VOICEMAIL", 131072);
define ( "VD_CALLENDPOINT_AUDIOTEXT", 262144);
define ( "VD_CALLENDPOINT_PAYPHONE", 524288);
define ( "VD_CALLENDPOINT_VPN", 1048576);
define ( "VD_CALLENDPOINT_FAX", 2097152);
define ( "VD_CALLENDPOINT_TETRA", 4194304);
...
function filters_call ( $filter, $parameters = array (), $buffer = array ())
{
  ...
  foreach ( $_filters[$filter] as $module => $function)
  {
    if ( ! function_exists ( $function))
    {
      ...
      if ( ! function_exists ( $function))
      {
        framework_error ( "VoIP Domain Framework: Registered function \"" . $function . "\" doesn't exist at filter \"" . $filter . "\"");
        continue;
      }
    }
    $_in["module"] = $module;
...
function framework_add_permission ( $permission, $description)
{
   ...
   */
  if ( in_array ( $permission, $_in["reservedpermissions"]))
  {
 ...
 *   boolean "documented"        -> If set false, the API will not be visible in
 ...
 * @param [string|array] $route The route(s) for this call
...
function framework_add_api_call ( $path, $route, $hook, $options = array ())
{
  ...
  /**
   * If route is an array, recursively call itself with all routes
   */
  if ( is_array ( $route))
  {
    $result = true;
    foreach ( $route as $newroute)
    {
      if ( ! framework_add_api_call ( $path, $newroute, $hook, $options))
      {
        $result = false;
      }
    }
  ...
  $filtered["documented"] = ( array_key_exists ( "documented", $options) && $options["documented"] === false ? false : true);
...
/**
 * Function to validate fields in a permitted list.
 *
 * @param string $fields List of fields to be validated.
 * @param string $permitted List of allowed fields.
 * @return boolean True if the fields are all valid.
 */
function api_filter_validate ( $fields, $permitted)
{
  /**
   * Validate parameters
   */
  ...
  if ( is_string ( $permitted))
  {
    $permitted = explode ( ",", $permitted);
  }
  if ( ! in_array ( "NULL", $permitted))
  {
    $permitted[] = "NULL";
  }

  return ! array_diff ( $fields, $permitted);
}
...
function api_call ( $path, $route, $parameters = array ())
{
  ...
  if ( $found)
  {
     ...
     * Call hook
     */
    return framework_call ( $entry[$route]["hook"], array_merge ( isset ( $vars) ? (array) $vars : array (), (array) $entry[$route]["variables"], (array) $parameters, array ( "api" => array ( "path" => $path, "route" => $route, "hook" => $entry[$route]["hook"], "parameters" => (array) $parameters, "vars" => isset ( $vars) ? (array) $vars : array (), "routevars" => (array) $entry[$route]["variables"]))));
  }
